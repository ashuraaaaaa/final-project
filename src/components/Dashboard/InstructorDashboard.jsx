...
const InstructorDashboard = ({ setScreen, currentUser, setModal }) => {
    ...
    const updateScore = (qId, type, value, maxPoints) => {
        ...
        setSelectedSubmission({ ...selectedSubmission, score: grandTotal, gradingDetails: newScores });
    };
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white p-4 sm:p-6 flex flex-col w-full max-w-5xl mx-auto">
                                    ...
                                    <tbody>
                                        {viewingSubmissions.map((sub, idx) => (
                                            <tr key={idx} className="border-b border-gray-700 hover:bg-gray-700">
                                                ...
                                                <td className="p-3">
                                                    <button onClick={() => openGradingModal(sub)} className="px-3 py-1 bg-blue-600 rounded text-xs font-bold hover:bg-blue-500 shadow">Grade / View</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                    ...
                    <div className="bg-gray-800 p-6 rounded-xl w-full max-w-6xl max-h-[95vh] overflow-y-auto border border-green-500 shadow-2xl">
                        <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
                            <div>
                                ...
                                <p className='text-gray-400 text-sm'>Scores update automatically as you type.</p>
                            </div>
                                        ...
                                        return (
                                            <tr key={qId} className="hover:bg-gray-750 transition-colors">
                                                ...
                                                <td className="p-4 align-top">
                                                                    ...
                                                                    <tr key={rIdx} className="border-b border-gray-700 last:border-0">
                                                                        ...
                                                                        <td className="py-2 w-1/3 text-right">
                                                                            <div className="flex items-center justify-end gap-1">
                                                                                <input 
                                                                                    type="number" min="0" max={r.points} 
                                                                                    ...
                                                                                    onChange={(e) => updateScore(qId, rIdx, e.target.value, r.points)}
                                                                                />
                                                    ...
                                                    {/* IDENTIFICATION GRADING (MANUAL INPUT) */}
                                                    {q.type === 'Identification' && (
                                                        <div className="flex flex-col gap-2">
                                                            ...
                                                            <div className="flex items-center justify-end gap-2 mt-2">
                                                                ...
                                                                <input 
                                                                    type="number" min="0" max={q.score} 
                                                                    className="w-20 p-2 bg-gray-700 text-white border border-blue-500 rounded text-center font-bold text-lg"
                                                                    value={rubricScores[`${qId}_manual`] || 0}
                                                                    onChange={(e) => updateScore(qId, 'manual', e.target.value, q.score)}
                                                                />
                                                                <span className="text-gray-500 font-bold">/ {q.score}</span>
                                                            </div>
                                                        </div>
                                                    )}

                                                    {/* AUTO-GRADED (READ ONLY) */}
                                                    {q.type !== 'Essay' && q.type !== 'Identification' && (
                                                        <div className="text-sm text-green-300 text-right mt-2">
                                                            <span className='text-gray-500 text-xs mr-2'>Auto-Graded</span>
                                                            <span className="font-bold text-white text-lg">
                                                                {(() => {
                                                                    const ans = (studentAns || "").trim();
                                                                    return ans === q.answer ? q.score : 0;
                                                                })()} / {q.score}
                                                            </span>
                                                        </div>
                                                    )}
            ...
            <div className="bg-gray-800 p-6 sm:p-8 rounded-xl shadow-2xl space-y-6">
                <h2 className="text-2xl font-semibold border-b pb-4 border-gray-700 text-green-400">Quiz Management</h2>
                <button className="w-full sm:w-auto px-6 py-3 bg-blue-600 rounded-lg hover:bg-blue-500 transition-colors font-bold shadow-md flex items-center justify-center gap-2" onClick={() => setScreen("createQuiz")}><span className='text-2xl'>+</span> Create New Quiz</button>
                            ...
                            {allQuizzes.filter(q => !q.isDeleted).map(quiz => {
                                ...
                                return (
                                    <tr key={quiz.id} className="border-b border-gray-700 hover:bg-gray-700 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-lg font-bold text-blue-300">{stats.totalSubmissions}</td>
