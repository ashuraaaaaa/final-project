...
import { loadQuizzes, saveQuizzes, deleteQuiz } from '../../utils/quizStorage.js'; // Added saveQuizzes
...
const InstructorDashboard = ({ setScreen, currentUser, setModal }) => {
    ...
    const getQuizStats = (quizId) => {
        ...
        return {
            ...
            // Check the main quiz object for release status, or fallback to submissions
    ...
    const handleReleaseResults = (quizId) => {
        // 1. Update Submissions (So students can see their specific score)
        const submissions = JSON.parse(localStorage.getItem(`quiz_submissions_${quizId}`) || '[]');
        ...
        saveQuizzes(updatedQuizzes); // Save to app_quizzes
        setAllQuizzes(updatedQuizzes); // Update UI

        setModal({ message: `Results released! Answers are visible to participants. New students can no longer join.`, type: "success" });
    }
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white p-6 flex flex-col w-full max-w-5xl mx-auto">
                                ...
                                <tbody>
                                    {viewingSubmissions.map((sub, idx) => (
                                        <tr key={idx} className="border-b border-gray-700 hover:bg-gray-700">
                                            ...
                                            <td className="p-3">
                                            <button onClick={() => setSelectedSubmission(sub)} className="px-3 py-1 bg-blue-600 rounded text-xs font-bold hover:bg-blue-500">View Answers</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
            ...
            {/* View Answers Modal (For Instructor to see specific student) */}
            {selectedSubmission && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 p-6 rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-green-500">
                        <div className="flex justify-between mb-4">
                            <h2 className="text-xl font-bold text-green-300">Answers – {selectedSubmission.studentName}</h2>
                            <button onClick={() => setSelectedSubmission(null)} className="px-3 py-1 bg-gray-600 rounded">Close</button>
                        </div>
                        {Object.entries(selectedSubmission.answers || {}).map(([qId, ans], idx) => (
                            <div key={qId} className="mb-4 p-4 bg-gray-700 rounded">
                                ...
                                <p className="text-yellow-300 mt-2">Student Answer:</p>
            ...
            <div className="bg-gray-800 p-8 rounded-xl shadow-2xl space-y-6">
                ...
                <button className="px-6 py-3 bg-blue-600 rounded-lg hover:bg-blue-500 transition-colors font-bold shadow-md flex items-center gap-2" onClick={() => setScreen("createQuiz")}>
                ...
                <div className='overflow-x-auto'>
                    <table className="w-full min-w-[700px] table-auto border-collapse rounded-lg overflow-hidden">
                        ...
                        <tbody>
                            {allQuizzes.length === 0 && <tr><td colSpan="5" className='text-center p-4 text-gray-400'>No quizzes created yet.</td></tr>}
                            {allQuizzes.map(quiz => {
                                ...
                                return (
                                    <tr key={quiz.id} className="border-b border-gray-700 hover:bg-gray-700 transition-colors">
                                        ...
                                        <td className="px-4 py-3 flex gap-2">
                                            ...
                                            <button onClick={() => handleReleaseResults(quiz.id)} className={`px-3 py-1 text-xs rounded font-bold ${stats.isReleased ? 'bg-gray-600 cursor-not-allowed' : 'bg-green-600 hover:bg-green-500'}`} disabled={stats.isReleased || stats.totalSubmissions === 0}>
